...
//
// The Smart Agent needs to run in the same task with the containers need
// to be monitored to get access to the data through ECS task metadata endpoint.
//
// ## Configuration from Labels
// You can configure monitors by putting special labels on your Docker
// containers in ECS task definition. You can either specify all of the
// configuration in container labels, or you can use the more traditional agent
// configuration with discovery rules and specify configuration overrides with labels.
//
...
func (o *ECS) discover() []services.Endpoint {
	...
	if response.StatusCode != http.StatusOK {
		logger.WithError(err).Error("Could not receive ECS metadata : %s ", http.StatusText(response.StatusCode))
		return nil
	}

	body, err := ioutil.ReadAll(response.Body)
	if err != nil {
		logger.WithError(err).Error("failed to read ECS metadata response body - %v", err)
		return nil
	}
...
func (o *ECS) endpointsForContainer(cont ecs.Container, taskDimensions map[string]string) []services.Endpoint {
	...
	if cont.KnownStatus == "RUNNING" {
		...
		// TODO : What if IPAddress doesn't exist?
